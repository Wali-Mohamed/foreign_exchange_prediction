...
class GarchModel:
    ...
    def __init__(self, from_curr,to_curr, repo, use_new_data):
        ...
        self.table_name=from_curr+'_'+to_curr
    ...
    def wrangle_data(self, n_observations):
        ...
        if self.use_new_data:
            ...
            api= AlphaVantageAPI()
            # Get data
            new_data= api.fx_daily(from_symbol=self.from_currency,to_symbol=self.to_currency)
            # Insert data into repo
            self.repo.insert_table(
                table_name=self.table_name, records=new_data, if_exists='replace'
        ...
        df =self.repo.read_table(table_name=self.table_name, limit=n_observations+1)
    ...
    def dump(self):
        ...
        filepath = os.path.join(self.model_directory, f'{timestamp}_{self.table_name}.pkl')
        # Save `self.model`
    ...
    def load(self):
        ...
        pattern = os.path.join(self.model_directory, f'*{self.table_name}.pkl')
        ...
        try:
            ...
            raise Exception(f'No model trained for "{self.table_name}".')
