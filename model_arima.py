...
class GarchModel:
    ...
    def wrangle_data(self, n_observations):
        ...
        if self.use_new_data:
            ...
            api= AlphaVantageAPI()
            # Get data
            new_data= api.fx_daily(from_symbol=self.ticker)
    ...
    def fit(self, p, q):
        ...
        self.model =ARIMA(self.data, order=(p,0,q)).fit()
    ...
    def __clean_prediction(self, prediction):
        ...
        # Create date range
        prediction_dates = pd.bdate_range(start=start_date, periods=len(prediction), freq='B')
    ...
    def dump(self):
        ...
        """
        # add absolute path
        path='C:\\Users\\ITC_Consultant\\Documents\\Personal Studies\\Applied Data Labs Similar Projects\\Project 8 stocks\\modules'
        # Create timestamp in ISO format
        timestamp = pd.Timestamp.now().isoformat()
        # Create filepath, including `self.model_directory`
        filepath = os.path.join(path,self.model_directory, f'{timestamp}_{self.ticker}.pkl')
        # Save `self.model`
